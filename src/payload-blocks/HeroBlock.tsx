...
export const HeroBlock: React.FC<HeroBlockProps> = ({ 
  title, highlightedTitle, subtitle, backgroundImage, links 
}) => {
  return (
                ...
                <div className="flex flex-wrap gap-x-12 gap-y-6">
                  {links.map((link, i) => (
                    <Link 
                      key={i}
                      href={link.url}
                      className="btn-theme !px-10 !py-4 transition-all"
                    >
                      <span className="text-sm uppercase tracking-[0.2em] font-bold">{link.label}</span>
                      <svg 
