...
import type { CompanyStat } from '@/payload-types'

export const StatsSection = ({ stats }: { stats: CompanyStat[] }) => {
  ...
  return (
    <section ref={sectionRef} className="py-20 bg-heading text-white">
      <div className="container mx-auto px-6">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 md:gap-12">
          {sortedStats.map((stat, idx) => (
            <div key={stat.id} className="text-center group hover:-translate-y-2 transition-transform duration-300">
              {stat.icon && (
                <div className={`mx-auto w-16 h-16 mb-4 flex items-center justify-center opacity-0 ${isVisible ? 'animate-fade-in-up' : ''}`} style={{ animationDelay: `${idx * 100}ms`, animationFillMode: 'forwards' }}>
                   <Media resource={stat.icon} imgClassName="w-10 h-10 object-contain filter brightness-0 invert drop-shadow-md" />
                </div>
              )}
              <div 
